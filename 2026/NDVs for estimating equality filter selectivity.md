...
Condition checks if NDV count is not zero and `target_interval`'s lower value is same as upper value then we assume our filter selectivity as `1/NDV`. 
...
Both of these reviews at the core address the problem of:
...
Our NDV count did not change as data flowed through both filters, same phenomenon can happen with different operators in the middle. We used our second filter as `y == 42` to prove first point of reviewer and we can use `y > 33 AND y < 42` to prove second point of reviewer.
